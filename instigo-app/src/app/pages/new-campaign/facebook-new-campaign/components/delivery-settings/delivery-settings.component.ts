...
import {
  ...
  optimizationGoals,
} from '../../facebook-new-campaign.data';

@Component({
  selector: 'ingo-nc-delivery-settings',
  templateUrl: './delivery-settings.component.html',
  styleUrls: ['./delivery-settings.component.scss'],
})
export class NCDeliverySettingsComponent implements OnInit, OnDestroy {
  ...
  private get campaignSettings(): FormGroup {
    return this.deliverySettings.parent.get('settings') as FormGroup;
  }
  ...
  private get adAccount(): AdAccountDTO {
    return this.campaignSettings.get('account').value;
  }

  ngOnInit() {
    this.parentForm = this.deliverySettings.parent as FormGroup;
    const campaignObjective = facebookCampaignObjectives.find(
      (item) => item.value === this.campaignSettings.value.objective,
    );
  ...
  private setTargetOptimizations(): void {
    const campaignOptimizationValue = this.campaignSettings.controls.objective.value;
  ...
  private getPixels(): void {
    const adAccount = this.campaignSettings.get('account').value;
    if (adAccount?.providerId) {
  ...
  private restrictBidStrategiesAccordingToCampaignObjective(campaignObjective: any): void {
